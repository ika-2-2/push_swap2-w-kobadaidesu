...
/*   Updated: 2026/06/30 16:29:46 by dakobaya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	op_rra(t_context *ctx)
{
	if (!can_rotate(ctx->stack_a))
		return ;
	rra(&ctx->stack_a, ctx->bench.total_count_flg);
...
void	op_rrb(t_context *ctx)
{
	if (!can_rotate(ctx->stack_b))
		return ;
	rrb(&ctx->stack_b, ctx->bench.total_count_flg);
...
void	op_rrr(t_context *ctx)
{
	if (!can_rotate(ctx->stack_a) && !can_rotate(ctx->stack_b))
		return ;
	rrr(&ctx->stack_a, &ctx->stack_b, ctx->bench.total_count_flg);
